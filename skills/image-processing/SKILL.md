...
description: Deterministically manipulate or transform existing image files - downscaling, cropping, format conversion, compression, filtering, edge detection, or scripted pixel pipelines - and choose between deterministic processing and AI generation. Use when the deliverable is a re-encoded or derived image file, or data extracted by code. For reading, describing, or answering questions about what an image shows, use the `see-images` skill instead.
---
