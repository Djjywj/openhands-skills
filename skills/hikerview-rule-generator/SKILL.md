...
> 2026-09-15 复盘新增。写源慢的根因通常**不是网络**（实测 curl 只占总耗时 12~20%，等待模型占 80~88%），
> 而是**用轮次堆**：一次回复只发一个请求、猜一个 URL 抓一次。本库实测一条规则曾打出 **698 次**零散请求、
> **199 条**"只发一个 URL"的命令，其中 46 条完全重复。

1. **一次抓齐，本地复用**：先把要探的地址（首页 / 各类目第 1 页 / 搜索页 / 详情页 / 候选接口）**列成一个清单，
   用一个循环一次抓完存盘**，之后只在**本地文件**上反复分析。
   **禁止**「猜一个 URL → 抓一次 → 看结果 → 再猜一个」的原地试错。
2. **不手抄样板**：探站命令里不要反复内联 UA / header。本技能自带 `scripts/fetch_url.py`
   （同步抓取、支持 GET/POST、自定义 UA 与 headers、自动跟随重定向、忽略 SSL），**直接用它**。
3. **同一形状的命令第 3 次出现时停手**，抽成循环 / 函数 / 脚本。
4. **先锁事实再写规则**：拿不准的行为（参数怎么传、路由走没走规则引擎、全角还是半角问号……）
   先做**一个能证伪的最小对照实验**，把结论写进一张事实表，再开始写 `rule.json`。
   一次对照实验胜过连改三版规则去试（本库有实测案例：两版无效迭代都撞在同一个报错上，
   而一次对照实验立刻定位了真因）。
5. **报进度**：探站 / 迭代超过约 40 次工具调用或 15 分钟，先给用户一个可见的中间产物（已确认的事实表、
   草稿规则、存下来的样本页），不要憋到最后一刻。
...
> 🚦 **无条件前置动作（不依赖用户怎么答）**：**无论用户手里有没有现成规则，动手分析站点之前都必须先查一次社区参照**——
> 见 `references/community_repos.md`（含各仓库定位、真实语料统计、GitHub 检索命令）。这一步**不是可选项**，
> 也不要用它去替代第 3 步的架构选择。实测本库有会话是在用户提醒之后才去查社区的，白多花了几十轮。
...
> 🤖 **用户选 B 时**，就靠上面那次社区检索的结果开写，别空手就动手。
