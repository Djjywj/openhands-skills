...
- 按点击位置用不同规则：`fyIndex`。
- 搜索的二级规则若与首页完全相同，直接写 `*`。
- **触发前提（最易错）**：列表项链接必须带请求修饰符（`;get;UTF-8;{referer@…}` / `;post;…`），海阔才会把它当
  「规则链接」交给规则引擎请求、执行 `detail_find_rule`；纯网页地址不带修饰符 → 直接开 WebView，规则不执行、不出选集。
  现成范式见 `examples/4e63v.rule.json`（列表项 url = 接口地址 + `;post;UTF-8;{headers}`）；详见 rule_format.md §5.3。
